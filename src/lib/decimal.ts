...
export function toFixed(val: number | string | Decimal, dp = 8) { return D(val).toFixed(dp) }
export function toNumber(val: number | string | Decimal) { return D(val).toNumber() }