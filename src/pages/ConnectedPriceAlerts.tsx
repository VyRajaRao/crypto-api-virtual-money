...
const ConnectedPriceAlerts = () => {
  const { state, createAlert, dispatch } = useSimulator();
  ...
  }, [state.alerts, searchQuery, filterStatus, filterPriority]);

  // Alert statistics
  const alertStats = useMemo(() => {
    const total = state.alerts.length;
    const active = state.alerts.filter(a => a.active).length;
    const triggered = state.alerts.filter(a => a.triggeredAt).length;
    const highPriority = state.alerts.filter(a => a.priority === 'high').length;
  ...
  const handleCreateAlert = async () => {
    ...
    const marketInfo = state.marketData.find(m => m.symbol === alertForm.symbol);
    if (!marketInfo) {
      setIsLoading(false);
      return;
    }
  ...
  const getConditionStatus = (alert: any) => {
    switch (alert.condition) {
      case 'above':
    ...
    <div className="container mx-auto p-4 space-y-6">
      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab}>
                ...
                <CardContent className="pt-12 pb-12">
                  <div className="text-center">
                    ...
                    <p className="text-muted-foreground mb-4">
                      Create your first price alert to get notified about market changes
                    </p>
        ...
        <TabsContent value="triggered" className="space-y-4">
          <div className="space-y-4">
            {state.alerts.filter(alert => alert.triggeredAt).length === 0 ? (
              <Card>
                <CardContent className="pt-12 pb-12">
                  <div className="text-center">
                    <Target className="mx-auto h-16 w-16 text-muted-foreground mb-4" />
                    <h3 className="text-xl font-semibold mb-2">No Triggered Alerts</h3>
                    <p className="text-muted-foreground">
              ...
              state.alerts.filter(alert => alert.triggeredAt).map((alert) => (
                ...
                <CardContent className="pt-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4 pt-4 border-t text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">
                        {state.alerts.filter(a => a.symbol === coin.symbol).length} active
                      </p>
                    </div>
                  </div>
...
export default ConnectedPriceAlerts;
